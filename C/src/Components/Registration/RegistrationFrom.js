...
const handleInputChange = (e) => {      // Called when input fields change. Updates the relevant field in the formData state
...
const validateEmail = (email) => {    // checks the format of an email address
const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
return emailRegex.test(email);
};

const isDuplicateNIC = (nic) => {     // check if a NIC alrealy exists
return students.some((student) => student.nic === nic);
};

const validateNIC = (nic) => {    // Validate NIC
if (nic.length === 10) {
