...
import PlacementTestDate from "../PlacementTest/PlacementTestDate";
import '../PlacementTest/unavailabledates.scss'
...
//   ];
  ...
  return (
    <div >
      ...
      {examResults.some((result) => result.stuId == {stuId}) ? (
        examResults.some((result) => result.stuId == {stuId} && result.state === "Faced") ? ( null  ) : (
     ...
     <label> Student ID : {stuId}</label> 
        {examResults.some((result) => result.stuId == {stuId}) ? (
          <ul>
            {examResults
              .filter((result) => result.stuId == {stuId}) 
              .map((result) => (
                <li key={result.availabilityId}>
                  Status: {result.state}, Marks: {result.marks}
                  {hasValidExamResult && (
                    <CourseDetails studentId={stuId} studentMarks={result.marks} />
                  )}
                </li>
              ))}
          </ul>
